...
model{
    ...
    for ( i in 1:N ) {
    	nu[i] = alpha_G + betaG_M*M_merge[i];
    	mu[i] = alpha_B + betaB_G*G_merge[i] + betaB_M*M_merge[i];
    }
    
    M_merge ~ multi_normal(rep_vector(0,N), K_M);    
    G_merge ~ multi_normal(nu             , K_G);
